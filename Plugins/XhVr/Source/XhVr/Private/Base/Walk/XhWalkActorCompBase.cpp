...
#include "Base/Walk/XhWalkActorCompBase.h"
...
{
	if (!IsRegisterCompleted() && !bIsWalking)
	{
		return;
	}
	Activate();
...
{
	if (!IsRegisterCompleted() && bCanTick)
	{
		return;
	}
	Deactivate();
				...
				if (PlayerCameraManager)
				{
					...
					XH_DELAY_B(1)
						...
						XH_DELAY_B(2)
							bIsWalking = false;
							//bStartWalk = false;
						XH_DELAY_E(2, FadeOutDuration)
					XH_DELAY_E(1, FadeInDuration)
